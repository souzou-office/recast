...
従来のバックオフィスシステムはマスターデータを人が入力・管理・更新する前提だった。recastは書類（登記簿・定款・株主名簿など）からAIが構造化データを再構成するため、マスターデータの入力・移行・メンテナンスが不要。書類を置くだけで動く。
...
**目的:** マスターシート（structured JSON）+ 書類雛形 → 書類一式を自動生成。人間はチェックだけ。

- **入力:** マスターシートのJSON + 案件フォルダ内の資料 + 書類雛形
- **出力:** 議事録・就任承諾書・届出書など書類一式
- **制御はプロンプト** — ルールベースのコードは書かない。AIが文脈から判断
  - 取締役が配列で2人 → 就任承諾書を2通生成
  - 案件フォルダに変更後定款あり → 「別紙の通り変更する」+ 別紙添付
  - 変更後定款なし → 変更内容を議事録に直接記載
  - 辞任+就任が混在 → 議案パーツを組み合わせ
- **議事録の構造:**
  - ベース（日時・場所・出席者・署名欄）は共通テンプレート1つ
  - 議案パーツ（選任・辞任・本店移転・増資・定款変更等）を個別に用意
  - マスターシートの内容に応じて議案パーツを選択・差し込み
- **通数の動的対応:** JSON配列の長さで書類の通数が決まる
- **将来:** 電子署名API連携（クラウドサイン・DocuSign等）で署名依頼まで自動化

### Phase 3 残タスク
- **雛形の手動登録UI** — 議事録ベース、議案パーツ（選任・辞任・本店移転・増資等）をテキスト貼り付けで登録
- **雛形のカテゴリ管理** — ベース/議案/承諾書/届出書等のカテゴリで分類
- **書類生成結果のプレビュー改善** — マークダウン表示、個別書類の区切り表示
...
**目的:** GPTのようにチャットをスレッド（会話）単位で管理。案件整理とは分離する。

- **チャット**: スレッドを複数作成可能。サイドバーにスレッド一覧表示、新規作成・削除・切り替え
- **スレッドタイトル**: 最初のメッセージからAIが自動生成
- **会社ごとのスレッド**: 会社を切り替えるとスレッド一覧も切り替わる
- **案件整理**: チャットとは独立した専用タブ。使い切り。結果はマスターシートとして保存され、Phase 3の書類生成に接続

### E. 案件整理タブの分離

**目的:** 案件整理をチャットから独立させ、専用タブにする。

- **案件整理タブ**: テンプレート選択 → 結果表示 → 根拠ファイルプレビュー → Phase 3書類生成へ
- **チャットタブ**: 純粋に質問・会話用（スレッド管理付き）
- **案件整理の結果はマスターシート（structured JSON）として保存** → Phase 3の入力になる
