...
## ★★ 2026-07-13 (続き3): 会話型ウィザード — 質問フェーズを一問一答のチャット形式に ★★

**ユーザーのUX方針（議論で確定）**: フォーム一覧は前時代的 → チャットボット風の一問一答。
選択式が背骨・自由入力は逃げ道（v1.5で「聞き手」AI＝自由文の解釈・まとめ答え・脱線対応を追加予定。
質問の発明はしない — 台本は木、AIは聞き方・解釈だけ、という役割分担で合意済み）。

**実装（v1 = 選択式ウィザード・AIゼロ・決定論のまま）**: `JireiWizard.tsx` 新規。
質問フェーズの既定表示が本体エリアの会話型に（左の一覧フォームは「一覧形式で入力する」で切替可）。
- 判断（choice）＝ボタン。プレフィルは「おすすめ」注記のみ（選択済みにしない＝提案止まり）
- 値＝入力欄。★同じ分岐条件の連続する値は1ターンにまとめる★（書面決議の日付4つ等。最大5個）
- 回答済みはログとして積まれ「直す」で再編集（波状の組み替えはエンジンが処理）
- ガードは会話の中に琥珀バブルで出現 / 導入バブルに読み取り済みサマリ（根拠付き・折りたたみ）
- ★生成は明示ボタンのみ★を API で保証: body.generate === true のときだけ生成。
  回答の再評価で最後の回答が揃っても自動生成しない（questions フェーズ + ready:true を返す）
- ★Reactの罠★: ターン部品をコンポーネント内で <X/> 定義すると毎レンダーで再マウントされ
  入力フォーカスが1文字ごとに飛ぶ → 関数呼び出し {renderX()} で描画する

**E2E（ブラウザ通し・本店移転）**: ウィザード表示→判断3つをボタンで（ガードが会話内に出現）→
値2つが1ターン→次へ→★自動生成されず最終ターンで停止★→生成ボタン→委任状1枚+ガード+プレビュー。
HMR後の下書き復元で会話の途中から再開できることも確認（消えない器）。

**（続き5）ウィザードを本物のページ遷移型に（ユーザー指摘「1ページ1ページ遷移するイメージ。
上からずーっと出しているやろ」= ログ積み上げ式は却下）**: 画面に出るのは★いまの質問だけ★。
答えると次のページに切り替わる（jirei-page-in の登場アニメ付き）。「← 戻る」で前の質問ページに
戻って直せる（選択中マーク表示・値は更新して進む）。「変更せず進む」で先へ。ヘッダーは
「N問回答済み・M問目」のみ。読み取り済みは1行の折りたたみ。回答ログの積み上げ表示は廃止。
viewIdx=null が「未回答の先頭に追従」、数値なら見直し中。回答すると null に戻して波状に任せる。

**（続き4）レイアウトを中央1カラム・ページ送りに全面変更（ユーザー指摘「何で左右に分けるの？
1ページごとにして。情報を重複させないで」）**: 申請タブの左右分割を廃止。
インボックス→提案（←資料に戻るリンク付き。提案表示中はインボックス・事由グリッドを隠す=1ページ1義）
→受付（必要な資料）→質問→生成結果+プレビュー（下に75vhで大きく）が全部同じ中央カラム（max-w-720px）
を1ページずつ進む。一覧形式（form）切替時も同じカラムにカードが並ぶ。
**バグ2件も同時修正**: ①ファイルのクリック選択が無反応（input.value を読む前にクリアして
FileList が空になる。受付と質問画面の両方）②受付を会話の1ページ目に統合し「◯◯には次の資料が
必要です」を最初に言う+受け取った資料の一覧を常時表示（入れたのに無視された、を作らない）。

## ★★ 2026-07-13 (続き): コンソール v2 — 木の編集 UI（ツリー画面に編集モード） ★★

**ユーザー要望**: 「ソフトで設定できるようにしないとみんな設定できない」→ JSON を開かずに
木を育てられる編集画面を実装。

**新規 `/api/jirei/edit`**: GET=生 JSON / PUT=検証して保存。保存前検証（when の参照先
questionId 実在・anyOf ⊆ choices・answer 出所の実在・id 重複・どこからも参照されない質問の警告
= loader 無検証の穴をここで塞ぐ）。旧版は data/jirei/history/<id>/ に自動バックアップ（gitignore済み）。

**JireiTreeView に編集モード**: 事由名・説明 / 質問（文言・選択肢・並び順↑↓・追加・削除）/
ガード（メッセージ・条件・追加・削除）/ 書類の出る条件 / 穴の出所
（資料から fact・質問で answer・固定値 const の付け替え。赤=出所なしをその場で潰せる）。
- ★選択肢のリネームは全域カスケード★: renameChoiceEverywhere が questions/documents/
  slots(配列含む)/guards の全 when を書き換える。「1字直すと分岐が全崩壊」を編集 UI が握り潰す。
  E2E: 「管轄外（法務局が変わる）」→「管轄外」で旧文字列残りゼロ・保存検証通過・リネーム後の生成9枚成功
- 削除は参照チェック付き（使用中の選択肢・質問は参照元一覧を出して拒否。E2Eで実証）
- 条件エディタ（CondEditor）: 語彙は 常に/条件1つ/すべて(かつ)/どれか(または) だけ。
  2段以上の入れ子は「JSONで編集」表示（実データには存在しない）
- 新規質問はどこからも参照されないと聞かれない → 保存時 warning で明示

**残課題**: 書類の追加・削除は未対応（テンプレファイルとセットなのでコンパイラ経由が正）/
配列バインディング（分岐で出所が変わる穴）は JSON 編集のまま / 質問の kind 変更は不可。

## ★★ 2026-07-13 (続き2): AIで枝分かれを仮生成 — 判断の層の下書き ★★

**ユーザー要望**: 「枝分かれは面倒だからAIで仮生成できるようにしたい」→
テンプレ逆算では書類に現れない判断（上層）が木に載らない構造的盲点への一般解を実装。

**`/api/jirei/suggest-branches`**: 現在の木+統一ルールを AI に渡し、判断（choice）→
従属質問→ガードの枝分かれ構造を提案させる。★ファイルには書かない = 仮生成★。
編集バッファに載り、ツリーでレビュー→保存で確定（捨てるのも自由）。
- 安全策: 既存質問の id/kind/choices はサーバー側で強制温存（スロットの when が参照。
  選択肢文言の変更はカスケード機能のある編集UIでやる）。AI が落とした既存質問は復元+警告。
  適用前に validateJirei、エラー時は1回だけエラーを見せて再生成。
- when の tool スキーマは再帰なし深さ1（原子 or all/any of 原子 = 実データの語彙と同じ）
- UI は2箇所: ①編集モードのヘッダー「AIで枝を仮生成」（木全体）②判断カードを開いた
  編集パネル内「この判断の下の枝をAIで仮生成」（focusQuestionId 付き = 文脈付き仮生成。
  その判断の各選択肢の従属質問・ガード・足りない選択肢を重点的に下書き）
- ガードも既存温存（AI提案からの削除は許さない。質問と同じ方針）。既存 choice への
  選択肢の★追加★は許可（既存値の削除・変更は禁止 = when が壊れないので追加は安全）
- プロンプトに「資料から自動で分かるものは質問禁止」の具体列挙（取締役の人数・株主の
  種別・定款の定め等）— 無いと「株主に法人はいますか？」のような fact を質問にしてくる

**E2E（取締役の辞任 = 分岐ゼロの木）**: 代取兼任か/退任原因/員数ゼロ/押印方法 等の判断5つ+
ガード7本（代取辞任届の雛形未登録・後任選任の同時申請・印鑑証明書…）を提案。
既存質問2つ温存・検証通過・ファイル無変更を確認。notes に専門家確認事項が付く。
validateJirei は src/lib/jirei/validate.ts に共有化（edit と suggest-branches が共用）。

**★まとめ文章→分岐表の一気生成★（ユーザー構想「分岐表が中核。一気にまとめた文章を
AIに投げて一気に作る」）**: suggest-branches に sourceText 追加 — 手続きの実務まとめ文章が
あれば★分岐表の正★として漏れなく木に落とす（文章に無い分岐の創作禁止・食い違いは文章優先）。
AI パネルに貼り付け欄。工程通し E2E 実証: 実務まとめ6行 → 判断3（代取兼任/後任代取=入れ子/
押印方式）+ガード6本（全部文章由来）→ 保存 warnings 0 → 波状質問（従属判断は選択に応じ
出没）→ 書類2枚・該当ガードのみ表示・未充足ゼロ。
文章→分岐表→保存→質問→生成の全工程が通る = 分岐表を文章から量産する道が開通。

## ★★ 2026-07-13: 本店移転を4判断に拡充 + 条件言語 all/any — 分岐の一般形 ★★

**ユーザー指摘**: 「圧倒的に分岐が足りない。定款は変わるか？管轄外か？取締役会設置か、
非設置なら取締役決定か株主総会かの選択もある」→ 木の構造を先に議論して確定
（①一本化=総会で全部決議、2段構えはしない ②取締役会の枝は雛形が無くても骨組みとして作る）。

**条件言語の拡張（エンジン・最小語彙）**: `JireiCondition` を再帰型に —
基本形 { questionId, anyOf } / { all: [...] }（かつ）/ { any: [...] }（または）。
語彙はこの2つだけに固定（増やすと専門家が木を読めない）。condOk 再帰評価 +
condQuestionIds（needed 収集の再帰対応）。既存の単一 when は無改修で動く。
★質問の連鎖で大半の AND は不要になる★（方式は総会が発生する分岐でしか聞かれない →
方式=書面 が暗黙に総会ありを含意）。any が本当に要るのは「総会あり = 定款変更が必要
または 総会で決めると選んだ」のような OR だけ。

**本店移転の4判断**: ①定款変更の要否（定款の定め×移転先。将来fact自動）→
②決定機関（不要のとき: 取締役の決定/株主総会で決める/取締役会=ガード）→
③決議方法（総会が発生する場合のみ波状で出現）→ ④管轄内/外。
書類: 書面決議一式(when ③) / 株主リスト(when any[①必要,②総会]) / 委任状(常時・管轄内外同文を確認済み) /
★印鑑カード交付申請書(when ④外)★。ガード3: 実開催・取締役会・移転決定書(all[①不要,②取締役])。

**印鑑カード交付申請書（グリッド様式・以前保留の難物）を実物から組み込み**:
scripts/build-kankatsugai-templates.mjs — op種別に exact（段落全文一致。裸の数字セル "8" を
電話番号と誤マッチさせない）と whole（文言を含む段落を丸ごと置換。全角ハイフン等の
文字種打ち間違い回避）を追加。セル分割の生年月日・委任日は★1質問→lineFieldで各セルに分配★
（氏名／住所の既存機構の流用。新機構なし）。facts に 代表取締役住所 追加（登記事項）。

**ツリー画面の多次元対応**: 主分岐（documents が最も参照する choice）でレーン分け、
主分岐以外の条件は書類カードに「出る条件: …」バッジ（humanizeCond で日本語化）。
テンプレの固定文言に黄色マーカーが付いているだけの偽「出所なし」は const スロット宣言で消す
（例: 印鑑カードの「令和」）。生成物のマーカーは produce の cleanupGeneratedDocx が全部剥がすので清書は無事。

**E2E（054・4ルート）**: 第1波=定款変更+管轄+移転先+移転日のみ / 不要→決定機関が波状出現 /
総会で決める→any条件で方式出現 / A:必要+書面+管轄内=8枚(回帰) / B:不要+取締役決定=委任状のみ+ガード /
C:必要+書面+管轄外=9枚・印鑑カード全充足【】残りゼロ / D:不要+総会+書面=株主リストがany条件で出た。

## ★★ 2026-07-10: 資料先行フロー（インボックス型）実装 — 入口の反転 ★★

**設計**: docs/資料先行フロー-設計.md（v2・3視点レビュー済み）。UXの尺度 = 人の手間を
翻訳/転記/判断/確認に分解し、判断と確認だけ残す。「事由を選んでから資料」を反転して
「資料を放り込む → 事由の方から名乗り出る → 判断だけ聞かれる」に。

**新規**:
- `/api/jirei/suggest` — 事由推定 + ★資料の種別分類★（AI境界①の拡張。sonnet-4-6、
  事由idはenum、候補ゼロ=該当なしを構造的に許可、資料一式ハッシュでキャッシュ
  data/suggest-cache/）。商号も抽出し登録済み会社との紐付けを提案（確定は人）。
- `/api/jirei/inbox` — 下書きの永続化（消えない器 §3-1）。data/jirei-inbox/draft.json に
  資料・メモ・推定結果・回答を自動保存。リロード/タブ切替で消えない。v1は進行中1件の割り切り。
- JireiPanel 全面組み替え: インボックス（ドロップ+電話メモ貼り付け→メモ.txt化）→
  提案カード（根拠引用+★前提=description+無条件guardsを確定前に表示★+充当表）→
  確定1クリック（sourcesConfirmed兼務）→ 質問3区画（判断/読み取り済み/入力）。
  該当なし時は a.手動選択 / b.覚えさせる(コンパイラ動線) / c.新種は1回目手作り。
  事由ボタンはフォールバックとして残置（details折りたたみ）。

**重要な挙動変更**:
- ★原本充当はAI種別分類が正★（kind付きはkindのみで照合、kind無しは従来の名前パターン）。
  客由来のファイル名（scan001.pdf）でも充当される。E2Eで実証。
- ★fact抽出の入力を「requiredSourcesに充当された資料だけ」に絞った★（旧: ドロップ全件。
  メールが混ざるとキャッシュが揺れ、案件連絡の値が原本の顔をするため）。
- ★質問フェーズのquestionsは activeQuestions（回答済み含む有効な質問全部）を返す★
  （旧: pending=未回答のみ→choiceが答えた瞬間消えて選び直せない）。生成ゲートは従来どおり
  pending。判断(choice)クリックで即再評価 → 従属質問が波状に入れ替わる（実開催⇄書面決議で実証）。
- プレフィルは全questionId分を保持し波状出現のたび適用。★choiceには自動適用しない★
  （提案表示止まり「資料には◯◯とあります」→確定は人のクリック。前提を無言で焼き込まない）。
- verify に依頼内容（intent=案件連絡ファイル）を渡し「事由レベルの当否」観点を追加。
- 既存5事由の description に「当たる依頼の言い回し」をバックフィル + コンパイラにも指示追加。

**E2E実証**（dev + node fetch + ブラウザ）: A=メール+原本(偽名リネーム)一括→取締役就任を
根拠付き提案・kind全部正解・054紐付け発見→確定→第1波=決議方法+方式非依存→メールから
7問プレフィル→10枚生成・未充足ゼロ / B=メモだけ→候補確定→会社レスで不足3点提示 /
C-1=原本のみ→候補0件(該当なしが言えた)。ブラウザ: リロード復元・判断切替の波状・ガード表示OK。

**残課題**: 生成済みdocxは下書きに保存していない（リロード後は再生成。答えは残る）/
eml未対応（貼り付け欄で代替）/ 複数事由の束は段1で複数提案までは可能だが生成は事由ごと。
