...
## ★★ 2026-07-22: 機械導出（derive）— 「専門家なら聞かずに埋める値」をエンジンに ★★

**ユーザー指摘**: 「委任状の日にちとか聞くと思うか？移転日聞いたらこっちで委任日とか作成日とか
埋めるんだよ。管轄も元の本店と移転先聞けばわかるんだからいちいちきくなよ」→ 木が「導出できる値」
を質問にしていた（分岐表の設計問題）+ エンジンに導出の語彙が無かった（構造問題）。両方を修正。

**エンジン: JireiQuestion.derive（src/lib/jirei/derive.ts・決定論・AIゼロ）**
- `{ kind: "copy", from: {answer|fact}, format?: "wareki-ymd" }` — 他の回答/事実のコピー
  （書面決議の日付4つ・委任日の既定値）。wareki-ymd = 「令和8年3月2日」→「8／3／2」（lineField分配用）
- `{ kind: "registry-office", address, compareTo?, same/different, part?: "bureau"|"branch" }` —
  商業登記の管轄判定。compareTo あり=2住所比較→choiceの値 / なし=法務局名(bureau)・支局名(branch。本局は全角スペース=様式空欄)
- 導出値はユーザー回答が無い所だけ埋める（人の上書きが常に勝つ）。導出できないとき
  （住所から判定不能等）は★従来どおり聞く★=安全側。API は effAnswers（導出込み）で全評価し
  `derived: {qid: {value, basis}}` を返す。pending は導出済みを除外（生成ゲートも通る）
- UI: ウィザードは導出済みの質問を★ページに出さない★。「こちらで埋めた値（N件）」の折りたたみに
  根拠付きで表示。一覧形式では「自動判定/自動で入れています」注記+クリック/入力で上書き
- validateJirei に derive の参照検証（コピー元の実在・choice 値の実在）追加

**★管轄表 data/jirei/houmukyoku.json — 都道府県1庁ではない！★**（Web調査・出典 houmukyoku.moj.go.jp）
- 集中化(2015完了)後も **41県=1庁 / 例外7都道府県=県内複数庁**。判定単位は「登記所」:
  東京23庁（区市町村単位！文京区→港区も管轄外）/ 神奈川2(本局=横浜・川崎, 湘南=他全域) /
  静岡3 / 愛知2(名古屋・岡崎) / 大阪4(本局・北大阪・東大阪・堺) / 福岡2(本局・北九州) /
  北海道4局・実質8登記所（釧路管内は集中化未実施: 本局・帯広・北見・根室・中標津が各自管轄）
- 表: 41県は文字列、複数庁は units（市区町村プレフィックス→識別名。長い一致優先=紋別郡遠軽町問題）
  ＋_default（神奈川・福岡のみ。ルールが完全な場合だけ）。units に無い住所=判定不能→聞く
- 令和7年4月21日〜 管轄外移転でも印鑑届書は不要（印鑑記録が移送）。印鑑カードは引き継がれず
  再請求 = 印鑑カード交付申請書は引き続き必要（現実装と整合）

**本店移転の木**: 質問順を 移転先・移転日→管轄 に並べ替え（導出の入力が先）。
kettei/teian/doui/minashi_date=copy(移転日) / kankatsu=registry-office比較(fact本店所在地×移転先) /
kankatsu_kyoku=bureau / kankatsu_shikyoku=branch / inkan_ininbi=copy(移転日, wareki-ymd)。
**E2E**: 管轄外ルートが★4問★（定款変更・決議方法・移転先+移転日・生年月日）で最終確認まで。
旧フローは9問。文京区→中央区=管轄内 / 文京区→港区=管轄外+支局「港」/ 大阪=管轄外+支局空欄。
生成9枚・未充足ゼロ・全導出値が正しく充足（全角日付のパースも）。

**申請タブの入口も1ページ化**（ユーザー指摘「これとか本当に1問一答していると思うの？」）:
資料投入ページには事由グリッドを出さない。「資料なしで進む — 事由を直接選ぶ」リンクで
事由選択ページ（グリッド+コンパイラ+←戻る）へ。該当なし時のフォールバック表示は従来どおり。

**残課題**: 編集UI（ツリー画面）は derive 未対応（JSON編集で）/ コンパイラ・仮生成のプロンプトに
derive の指針未追加（「導出できる値は質問にせず derive で」）/ 愛知・北海道の郡部は管轄表が
部分的（裏取りできた市郡のみ。無い住所は質問に落ちるので誤判定はしない）/
書面決議の日付既定値=移転日と同日（事務所の慣行が違うなら木の derive を offset 対応に拡張）
