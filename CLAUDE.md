...
### verify（チェック）の改修 — 次の実装
現状、verify は「案件整理 vs ディスク上の既存ファイル」しか突き合わせておらず、**recastが生成したdocxはチェック対象に含まれていない**。そのため、生成書類内の誤字・踊り字の誤り（「個個」→「個々」等）を検出できない。

- **B2: 生成済みdocxをverifyの入力に含める**
  - `/api/verify` のリクエストボディに `generatedDocs?: { fileName, docxBase64 }[]` を追加
  - サーバー側で mammoth を使ってテキスト抽出、プロンプトに混ぜる
  - `VerificationView.tsx` 側で、現在の会社/案件の `generatedDocuments` を無条件に全部送る
  - 副次効果: ファイルパス依存を減らし、Web展開への布石になる
- **C: verifyプロンプトに日本語表記のチェック観点を追加**
  - 踊り字の誤り（「個個」→「個々」、「日日」→「日々」等）
  - 明らかな誤字脱字・表記ゆれ
  - 重要度 🔵軽微 で報告
- **注意: produce側で先回り対策（ルールベース正規化等）は入れない**
  - 責任分割: produce=生成 / verify=品質管理
  - LLMによる判定に集約する方が網羅的かつ保守的
...
- `claude/review-document-creation-CRLlT` — 書類生成まわりの設計議論 + verify改修（B2/C）
