...
- **基本情報の型定義:**
  ...
  CompanyProfile {
    ...
    変更履歴: { 日付: string, 内容: string, 根拠ファイル: string }[]
    updatedAt: string
    sourceFiles: ...
  }
  ```
- **変更履歴:** 共通フォルダの過去ファイル（旧登記簿・旧定款等）も読み、時系列の変更履歴をstructuredに含める
  - 初回: 全ファイル読んで変更履歴を生成
  - 以降: 新規追加ファイルの差分だけ読んで追記
  - 例: `[{ 日付: "2023-04-01", 内容: "本店移転（東京都千代田区→港区）", 根拠ファイル: "履歴事項全部証明書_20230401.pdf" }]`
- **マスターシート（テンプレート実行結果）:** テンプレートのitemsがJSONのキーになる。業務ごとの必要情報を構造化して保存
  - 例（役員就任）: `{ サマリー: "...", 新取締役: { 氏名: "...", 住所: "..." }, スケジュール: "..." }`
- **横断検索:** 新tool `search_all_companies` → 全社の `structured`（変更履歴含む）を渡す
