...
1. 指摘の問題箇所が、その項目のラベル名と **明確に同じ概念**を指している
   - 例: ラベル「代表取締役氏名」、指摘「代表取締役の氏名が間違っている」 → ✓
2. 指摘の expected の値の型と、その項目の現在値の型が **一致**する
   - 例: ラベル「氏名」現在値「福田峻介」、expected「三上春香」 → ✓ 型一致（人名）
   - 例: ラベル「氏名」現在値「福田峻介」、expected「94,500」 → ✗ 型不一致（人名 vs 数値）
3. 同じ slotId は1つの issue にしか付けない（重複禁止）

#### 紐付けてはいけないケース
- 指摘文に項目の現在値（人名、会社名等）が **言及されているだけ** で、指摘の本体は別の項目（例: その人の株数や住所）
  - ✗ 例: 指摘「**三上春香**の株数が違う」、ラベル「氏名」（現在値「三上春香」）→ 「三上春香」は文脈で出てるだけ。指摘の本体は **株数** であって 氏名 ではない
  - 正解は「持株数」みたいな項目に紐付けるか、該当項目がなければ slotId を省略
