...
書類ごとに「edit list」を返してください。edit のタイプは 4 種類:
  ...
  "documents": [
    {
      ...
      "edits": [
        ...
        { "type": "replace", "old": "50000", "new": "49000" },
        {
          ...
          "old": "令和８年２月１１日",
          "new": "令和８年５月２１日",
          "contextBefore": "取締役決定書",
          "contextAfter": "取締役の全員の一致"
        },
        {
          "type": "delete-section",
...
- **replace**: \`old\` の文字列を \`new\` に置換。氏名揺れ、数値ミス、住所違い等の単純な置換に使う。
  - 同じ \`old\` が複数箇所に出る場合は **contextBefore / contextAfter** で**前後の文脈**を指定して場所を絞り込めること
  - 例: 「令和８年２月１１日」が冒頭と末尾の 2 箇所にあり、冒頭だけ差し替えたい → \`contextBefore: "取締役決定書"\` で限定
  - contextBefore/After が指定された場合、その文脈を含む段落の \`old\` だけが置換される
  - 文脈ヒント無しの場合は**全 \`old\` を置換**する (全箇所同じ値にしたい場合)
- **delete-section** ← **議案ブロック等の複数段落を一括削除するときはこれを使う (推奨)**
  - \`anchor\` 段落から \`endAnchor\` を含む段落の**直前まで**を一括削除。docx 専用
  - 例: 議案2 ブロック (見出し + 条文 + ア + イ + ウ + 「記」) を全消し
    → \`anchor: "議案２（取締役の報酬に関する件）", endAnchor: "議案３"\`
  - endAnchor 省略時は anchor 以降文書末尾まで消える (危険なので極力指定)
- **delete-paragraph**: \`anchor\` 文字列を含む**段落 1 つだけ**を削除。docx 専用
  - 単一段落 (空行、不要な但し書き 1 行) のみ
  - 議案ブロックのように複数段落の意味単位を消すには使わないこと (見出しだけ消えて条文が残る半端な結果になる)
