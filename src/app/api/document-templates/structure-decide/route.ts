...
### edit タイプ
- **delete-paragraph**: \`anchor\` 文字列を含む**段落 (=改行までの 1 ブロック) を丸ごと削除**。docx 専用
  - 議案ブロック削除、空行削除、不要な但し書き削除等

### 必須フィールド
- **anchor**: 削除対象段落を**一意に特定できる文字列**を指定。短すぎると別箇所を巻き込む
  - ❌ 「議案」「年月日」等の一般的すぎる短い文字列
  - ✅ 「議案２（取締役の報酬に関する件）」「第3条 取締役の任期」等の具体的なフレーズ
- **expectedMatches**: その anchor がテンプレ内で**何段落 hit するはず**かを宣言。
  - 通常は 1 (1 個の段落だけ消したい場合)
