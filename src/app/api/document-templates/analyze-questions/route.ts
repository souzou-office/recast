...
## 質問する前に必ず原本を確認すること (定款型の定型事項) — 最優先ルール
代表取締役の決定方法 / 株式の譲渡制限の承認機関 / 公告方法 / 株式の内容・種類 / 機関設計 /
役員の任期 などは、通常「定款」や「履歴事項全部証明書」に書かれている定型事項。
これらが基本情報・案件整理結果に見当たらなくても、**「どこにも書かれていない(下記ケース3)」と判定して
質問する前に、必ず read_common_file で定款/履歴事項全部証明書の原本を読んで確認すること。**
...
submit_clarification_questions tool で質問リストを返してください。`;
  ...
  const profileSourcesSet = company.profileSources && company.profileSources.length > 0
    ? new Set(company.profileSources) : null;
  ...
  const commonFilesBlock = commonFiles.length > 0
    ? `\n## 共通フォルダの原本ファイル一覧（read_common_file で読める）\n質問する前に、原本で確定できる点はここを読んで確認すること。基本情報・案件整理結果に既にある内容は読み直さない。\n${commonFiles.map((f) => `- [${f.folderName}] ${f.name}\n  path: ${f.path}`).join("\n")}\n`
    : "";
