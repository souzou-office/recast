...
6. **予備欄・任意の補足欄 (通常は空欄でよい欄)**
   - ラベルが「補足事項」「追記事項」「備考」「前文」「その他」「特記事項」等で、
     かつ書式ヒントも前案件値も無い (テンプレ作成者が念のため空けてある欄) → **質問せず空欄のまま**にする
   - 例: 「証明書の前文・補足事項」「証明書作成日の追記事項」← これらは通常空欄。聞かれてもユーザーは困る
   - 判断基準: 「この欄を空欄にしても書類は完成するか?」→ Yes なら任意欄 → 質問しない

7. **株主リスト等の連番行で、その案件には該当者がいない行**
   - 「議決権上位株主5〜10の氏名/住所/株式数」のように、株主数より多い予備行は **空欄のまま**。質問しない
   - (株主データは Phase 1 整理結果から自動で必要数だけ埋まる)
