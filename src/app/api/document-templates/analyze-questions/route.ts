...
// 原本ファイルを読む tool (案件整理 execute と同じ)。質問を作る前に AI が定款等の原本を
// 読んで、読めば確定できる点はわざわざ質問せず、本当に人間判断が要る点だけ質問にするため。
const READ_COMMON_FILE_TOOL = {
  name: "read_common_file",
  description: `会社の共通フォルダ（定款・登記簿・株主名簿等）にある原本ファイルの内容を取得する。
質問を作る前に、原本を読めば確定できる点(例: 定款の代表取締役の決定方法、譲渡制限の承認機関)は
この tool で確認し、本当に人間の判断が要る点だけを質問にすること。
ただし基本情報(profile)や案件整理結果に既に書かれている内容を読み直すのは無駄なので呼ばない。`,
  input_schema: {
    type: "object" as const,
    properties: {
      path: { type: "string", description: "読み込むファイルの絶対パス。共通ファイル一覧に列挙されたパスから1つ選ぶ。" },
    },
    required: ["path"],
  },
};

// 共通フォルダ配下のファイルパスを再帰収集 (内容は読まない)
async function walkPathsOnly(dir: string, out: { path: string; name: string }[]): Promise<void> {
  let entries;
  try { entries = await listFiles(dir); } catch { return; }
  for (const e of entries) {
    if (e.isDirectory) await walkPathsOnly(e.path, out);
    else out.push({ path: e.path, name: e.name });
  }
}
...
## 質問する前に必ず原本を確認すること — 最優先ルール
「どこにも書かれていない(下記ケース3)」と判定して質問する前に、以下は必ず read_common_file で
原本を読んで確認すること。**read_common_file は画像(マイナンバーカード等の JPG/PNG)も読める。**
原本に書いてあれば質問せず、その値をそのまま使う。原本を読んでも確定できない時だけ質問する。

(A) 定款・登記で確定する定型事項: 代表取締役の決定方法 / 株式の譲渡制限の承認機関 / 公告方法 /
    株式の内容・種類 / 機関設計 / 役員の任期 など → 定款・履歴事項全部証明書 を読む。

(B) 人の属性 (生年月日 / 住所 / 氏名の正確な表記 など): マイナンバーカード・運転免許証・
    印鑑証明書・住民票 などの本人確認書類に書かれている (JPG/PNG/PDF のスキャン画像が多い)。
    **生年月日・住所はまさにこれらの画像に写っている**ので、聞く前に必ず該当書類を read_common_file で
    開いて画像として読むこと。「画像だから読めない」と決めつけて質問してはいけない。
    ★住所の一致確認も同じ★: 「カードの住所が登記と一致するか」は**お前がカードを読んで照合する**仕事。
    ユーザーに「一致していますか?」と聞かない (下記「質問にしてはいけないケース」6 を厳守)。
...
6. **「資料Aと資料Bは一致していますか?」という突き合わせの代行要求** ← 特に厳禁
   - 2つの資料の値が一致するかの照合は **AI 自身の仕事**。ユーザーにやらせない (recast は事実チェックをする側)。
   - ✕ 悪い例:「マイナンバーカードの住所は登記と一致していますか?」
     → カードは JPG で read_common_file で読めるのだから、**自分で開いて住所を読み、登記と照合しろ**。
   - 読んで **一致** → 質問せずその値を使う (確認の質問を作らない)。
   - 読んで **実際に食い違っていた** 時だけ、ケース1として「カードは『A』、登記は『B』。どちらを使うか?」と
     **両方の実際の値**を示して質問する。候補の出典は実際に読んだ資料名を書く (カードを読んだなら [マイナンバーカード])。
   - 「念のため」で一致確認の質問を作らない。読めば済む話を人に振らない。
...
submit_clarification_questions tool で質問リストを返してください。`;
  ...
  let caseFolderPath: string | null = null;  // 選択中の案件フォルダ (read_common_file の対象に追加)
  let caseDisabled: string[] = [];
  try {
    ...
    caseFolderPath = threadData.folderPath || null;
    caseDisabled = threadData.disabledFiles || [];
  } catch { /* ignore */ }
  ...
  // 質問生成が原本を読めるよう、共通フォルダのファイル一覧を作る (read_common_file の対象)。
  // ★profileSources (基本情報の参照ファイル選択) では絞らない★: あれは「基本情報生成に使う
  // ファイル」の選択であって、ここの read_common_file は「迷った時に原本を確認する」用途。
  // マイナンバーカード等を基本情報の選択から外していても、生年月日確認のために読めるべき。
  // (除外設定 disabledFiles だけは尊重する。)
  ...
  for (const sub of company.subfolders.filter((s) => s.role === "common")) {
    ...
    await walkPathsOnly(sub.id, subFiles);
  ...
  // 案件フォルダの原本も read_common_file で読めるようにする。生年月日のマイナンバーカード等が
  // 共通でなく案件フォルダに入っているケースに対応 (案件整理は画像を見ているが、その値が
  // organizeResult に文章化されていないと質問生成からは見えないため、ここで直接読めるようにする)。
  if (caseFolderPath) {
    try {
      const caseFiles: { path: string; name: string }[] = [];
      await walkPathsOnly(caseFolderPath, caseFiles);
      const caseName = caseFolderPath.split(/[\\/]/).filter(Boolean).pop() || "案件";
      for (const f of caseFiles) {
        if (isPathDisabled(f.path, caseDisabled)) continue;
        if (allowedCommonPaths.has(f.path)) continue;
        commonFiles.push({ path: f.path, name: f.name, folderName: `案件:${caseName}` });
        allowedCommonPaths.add(f.path);
      }
    } catch { /* 案件フォルダが読めなくても無視 */ }
  }
  const commonFilesBlock = commonFiles.length > 0
    ? `\n## 原本ファイル一覧（共通フォルダ＋案件フォルダ。read_common_file で読める。JPG/PNG等の画像も読める）\n質問する前に、原本で確定できる点はここを読んで確認すること。特に生年月日・住所はマイナンバーカード等の画像に写っている。基本情報・案件整理結果に既にある内容は読み直さない。\n${commonFiles.map((f) => `- [${f.folderName}] ${f.name}\n  path: ${f.path}`).join("\n")}\n`
    : "";

  // テンプレ群メモ + 案件フォルダメモ(.txt/.md) を質問生成にも渡す (穴埋め(analyze)と共通の loadMemoNotes)。
  // メモに「この案件/このテンプレはこう扱う」と書いてあれば、聞くべき/聞かざるべき質問の判断に効く。
  let memoBlock = "";
  try {
    const { loadMemoNotes } = await import("@/lib/memo-notes");
    memoBlock = await loadMemoNotes(templateFolderPath, caseFolderPath);
  } catch { /* メモが読めなくても続行 */ }

  try {
    // read_common_file で原本を読みつつ、最後に submit_clarification_questions を出す tool-use ループ。
    // 「質問する前に原本を確認 → 読めば分かる無駄な質問を減らす」ため (案件整理と同じ仕組み)。
    ...
    let workingMessages: Anthropic.MessageParam[] = [{
      ...
      content: [
        ...
        // 書類別 suffix: 質問抽出指示 + メモ(注意事項) + テンプレ本文 + 共通ファイル一覧
        { type: "text", text: promptText + memoBlock + commonFilesBlock },
      ],
    }];
    ...
    for (let iter = 0; iter < MAX_ITER; iter++) {
      const resp = await client.messages.create({
        ...
        max_tokens: 8192,
        temperature: 0,
        tools: [READ_COMMON_FILE_TOOL, CLARIFICATION_TOOL],
        // 最終 iter は質問提出を強制 (原本の読みすぎ・無限ループ防止)
        tool_choice: iter < MAX_ITER - 1 ? { type: "auto" } : { type: "tool", name: "submit_clarification_questions" },
        messages: workingMessages,
      });
      logTokenUsage(`/api/document-templates/analyze-questions (iter=${iter})`, MODEL, resp.usage);
      const submitted = resp.content.some((b) => b.type === "tool_use" && b.name === "submit_clarification_questions");
      if (submitted || resp.stop_reason !== "tool_use") { response = resp; break; }
      // read_common_file を実行して tool_result を返す
      workingMessages = [...workingMessages, { role: "assistant", content: resp.content }];
      const toolResults: Anthropic.ToolResultBlockParam[] = [];
      for (const block of resp.content) {
        if (block.type !== "tool_use" || block.name !== "read_common_file") continue;
        const reqPath = (block.input as { path?: string })?.path || "";
        const fileName = reqPath.split(/[\\/]/).pop() || reqPath;
        if (!allowedCommonPaths.has(reqPath)) {
          toolResults.push({ type: "tool_result", tool_use_id: block.id, content: `エラー: 指定パスは共通ファイル一覧にありません: ${reqPath}` });
          continue;
        }
        try {
          const fc = await readFileContent(reqPath);
          if (!fc) {
            toolResults.push({ type: "tool_result", tool_use_id: block.id, content: `エラー: 読み込み失敗: ${reqPath}` });
            ...
            toolResults.push({ type: "tool_result", tool_use_id: block.id, content: `(${fileName} はテキスト抽出できない形式です)` });
          } else {
    ...
    if (!response) return NextResponse.json({ questions: [] });
