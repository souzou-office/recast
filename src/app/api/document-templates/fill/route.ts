...
2. **slotKey は ★ラベル★ の中身のみ** (★ や 【】 は付けない)。
3. **削除指示の回答 (例: 「議案2 自体を削除」「報酬なし」) は delete op で表現**。modify の value に「【議案2 削除】」のような指示語を流し込むな。
4. **不明な値は空文字 modify** で返す (\`value: ""\`)。チェック段階で AI が読み直して埋めるか、ユーザー手動修正に倒す。
5. **複数書類間で同じ意味の値は揃える** (★ラベル★ が同じならサーバーが同じ値を入れるので、書類間の不一致は構造的に発生しないはず)。
6. **議案番号の繰り上げ**: 議案2 を delete したら、議案3 のラベルを議案2 に変える必要があるなら **modify or delete + insert で表現** すること。サーバーは自動で繰り上げない。
