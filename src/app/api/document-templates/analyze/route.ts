...
## value に何を入れるか (重要・必ず守る)

value には **slot に入れる「実データだけ」** を書く。テンプレに既に書かれている肩書き・ラベル・
単位・敬称等を **絶対に含めない**。produce は value をテンプレに **そのまま挿入** するため、
重複すると「取締役 取締役 古澤」のような二重出力になる。

判断基準: 「テンプレを目で見て、この slot 位置に value をそのまま貼り付けたら自然な文になるか？」
を毎回考える。

### NG (これは絶対やらない)

| slot | 悪い value | 良い value |
|---|---|---|
| 選任される取締役1の氏名 | "取締役　　古澤　利成" | "古澤　利成" |
| 代表取締役の住所 | "代表取締役　神奈川県横浜市…" | "神奈川県横浜市…" |
| 代表取締役の氏名 | "代表取締役　古澤　利成" | "古澤　利成" |
| 資本金 | "金 1,000,000 円" | "1,000,000" |
| 増加株式数 | "100 株" | "100" |
| 募集事項 (発行価格) | "1株につき金 10,000 円" | "10,000" |
| 発行可能株式総数 | "1,000株" | "1,000" |

### 判断のヒント

- slot ラベルが「氏名」「住所」「金額」等で **何の項目か** を表しているなら、value は **値だけ**
- テンプレ本文を見て、slot の **直前直後** に「取締役」「代表取締役」「金」「円」「株」「氏」等が
  既に書かれていたら、それを value に含めない
- 例外: 役職込みで slot 名が "役職欄" のように **役職そのものを入れる枠** ならそれは入れる。
  ただしテンプレ本文を見て判断すること

判断に迷ったら value から余分な接頭辞を **削る** 方を選ぶ (足すより削る方が安全)。`;
