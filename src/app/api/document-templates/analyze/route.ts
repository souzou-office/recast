...
典型例: 引受人が株式会社なのにテンプレが組合フォーマットで「本店」「商号」行が無い → 追加する。

\`\`\`json
{
  "afterSlot": "乙の名称",                            // この slot を含む行の直後に挿入
  "template": "本店　★乙の本店所在地★",               // 行のテンプレ文字列
  "fills": [
    { "slot": "乙の本店所在地", "value": "東京都...", "source": "基本情報" }
  ],
  "reason": "..."
}
\`\`\`
...
- \`template\` に書いた **★label★ は必ず同じエントリの fills に対応 entry が要る**
  (★乙の本店所在地★ を書いたら fills に \`{slot:"乙の本店所在地", value:...}\` を必ず入れる)
- \`afterSlot\` はテンプレに既存の slot 名 (新規挿入する行の slot 名ではない)
- 固定文 (注釈・定型句) を足したいだけならテンプレ側に書くべき。rowInsertions は使わない

**🚨 重要: 既存ラベルがあれば rowInsertions は出すな (重複行事故防止)**:

rowInsertions を出す前に、**テンプレに同じ意味のラベルの行が既にあるかチェック** する。
あれば rowInsertions は出さず、その既存 slot に **fill で値を入れるだけ** で済む。

例: 引受人が株式会社で、テンプレが組合フォーマット (主たる事務所/名称/無限責任組合員/組合員/代表取締役) のとき:

| テンプレの行 | 株式会社で必要なラベル | 処理 |
|---|---|---|
| 主たる事務所 ★...★ | 本店 | delete-row + rowInsertions (ラベル違う) |
| 名称 ★...★ | 商号 | delete-row + rowInsertions (ラベル違う) |
| 無限責任組合員 ★...★ | (不要) | delete-row のみ |
| 組合員 ★...★ | (不要) | delete-row のみ |
| **代表取締役 ★...★** | **代表取締役** | **fill のみ (ラベル同じ、rowInsertions 不要)** |

最後の行に注意。「代表取締役」ラベルは両フォーマットで共通なので、**rowInsertions で代表取締役行を
追加してはいけない**。既存 slot を fill するだけ。これを忘れると **代表取締役 行が 2 つ出力される事故** になる。

**ラベル変換のパターン (例: 主たる事務所/名称/無限責任組合員/組合員/代表取締役 → 本店/商号/代表取締役)**:

\`\`\`json
"slotDecisions": [
  { "slot": "乙の主たる事務所", "action": "delete-row", "reason": "ラベル変更 (主たる事務所→本店)" },
  { "slot": "乙の名称",         "action": "delete-row", "reason": "ラベル変更 (名称→商号)" },
  { "slot": "乙の無限責任組合員", "action": "delete-row", "reason": "株式会社では不要" },
  { "slot": "乙の組合員",       "action": "delete-row", "reason": "株式会社では不要" },
  { "slot": "乙の代表取締役",   "action": "fill", "value": "川上 登福", "source": "..." }
],
"rowInsertions": [
  {
...
(代表取締役 行は slotDecisions の fill で処理。rowInsertions に入れない)
