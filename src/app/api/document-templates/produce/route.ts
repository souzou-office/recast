...
export async function POST(request: NextRequest) {
  const { companyId, templateFolderPath, mode, caseRoomId, masterContent: directMasterContent, confirmedAnswers: rawConfirmedAnswers, folderPath: caseFolderPath, disabledFiles: caseDisabledFiles } = await request.json() as {
    companyId: string;
    templateFolderPath: string;
    mode?: "fill" | "generate";
    caseRoomId?: string;
    masterContent?: string;
    // 後方互換: 旧形式 Record<placeholder, answer> も受け取る
    confirmedAnswers?:
  ...
  const confirmedQA: QAEntry[] = Array.isArray(rawConfirmedAnswers)
    ...
    : rawConfirmedAnswers && typeof rawConfirmedAnswers === "object"
      ...
      : [];
  // 既存コードが confirmedAnswers (Record形式) を参照しているので、簡単な検索用 Record を残す
  const confirmedAnswers: Record<string, string> = {};
  for (const qa of confirmedQA) confirmedAnswers[qa.placeholder] = qa.answer;

  // プロンプトに「質問文 + 回答」の形で渡すための整形
  const renderQABlock = (heading: string): string => {
  ...
  if (docxFiles.length > 0 && mode !== "generate") {
    ...
    if (allPlaceholders.size === 0) {
      ...
      const documentsHL: DocOut[] = [];

      // テンプレの .labels.json を読み込んで slotId → ラベル情報のマップを返すヘルパー
      const loadSlotLabelsFor = async (templateFilePath: string): Promise<Map<number, { label: string; format: string; sourceHint?: string }>> => {
          ...
          if (ext === "xlsx" || ext === "xls") {
            ...
            if (xlPlaceholders.length === 0) {
              ...
              try {
                ...
                // system = 全書類で共通（案件データ・ルール・出力形式）
                // user = 書類固有（Excelテンプレート本文）
                const xlMarkerSystem = `あなたは司法書士事務所の書類作成担当です。
Excelの書類テンプレートに含まれる ［要入力_N《前案件:○○》］ と書かれた部分が「可変箇所」です。
...
## セル内の固定文言の保持（最重要）
Excel のセルは「固定文言＋可変値」が1つのセルに混在していることが多い。
hint（前案件の値）に「ラベル」や「単位」のような固定部分が含まれている場合、
**その固定部分はそのまま保持して、値部分だけ差し替える**こと。

- 例1: hint が \`氏名：福田峻介\` → 返答 \`氏名：三上春香\`（"氏名：" を残す）
- 例2: hint が \`会社名　株式会社JINGS\` → 返答 \`会社名　株式会社ABC\`
- 例3: hint が \`第1株主　福田峻介　100株\` → 返答 \`第1株主　三上春香　500株\`（見出しラベル・構造を維持）
- 例4: hint が \`2026年2月9日\` → 返答 \`令和８年２月１日\`（フォーマット自体は変えてよいが、構造は類似）
- 例5: hint が \`福田峻介\`（ラベルなし）→ 返答 \`三上春香\`（値のみ）

判定のコツ: hint を「ラベル/接頭辞」＋「値」に心の中で分解し、**値の部分のみ** を今回の案件データで入れ替える。
「ラベル/接頭辞」が判然としない場合は hint の構造をそのまま踏襲するのが安全。
...
- 基本情報・案件整理テキストに見つからない値だけ "（要確認）"`;
                ...
                const xlMarkerUser = `## Excelテンプレート
${xlMarkedText}

この Excel の ［要入力_N］ に対する値を JSON で返してください。`;

                const xlResponse = await client.messages.create({
                  model: MODEL,
                  max_tokens: 4096,
                  system: [
                    { type: "text", text: xlMarkerSystem, cache_control: { type: "ephemeral" } },
                  ],
                  messages: [{ role: "user", content: xlMarkerUser }],
                });
                logTokenUsage("/api/document-templates/produce#xlMarker", MODEL, xlResponse.usage);
                ...
                if (xlJsonMatch) {
                  ...
                  // 同時に filledSlots（slotId + label + value）も構築。
                  const xlReplacements: Record<string, string> = {};
                  const xlFilledSlots: FilledSlotOut[] = [];
                  const xlSlotLabels = await loadSlotLabelsFor(df.path);
                  for (const [k, v] of Object.entries(xlParsed)) {
                    ...
                    const meta = xlSlotLabels.get(id);
                    xlFilledSlots.push({
                      slotId: id,
                  ...
                  documentsHL.push({
                    name: baseName,
                    docxBase64: xlOutBuf.toString("base64"),
                    previewHtml: "",
                    fileName: xlOutName,
                    templatePath: df.path,
...
JSONのみ返してください。データにない情報は "（要確認）" としてください。`;
            ...
            try {
              ...
              logTokenUsage("/api/document-templates/produce#xlPlaceholder", MODEL, xlResponse.usage);
              const xlText = xlResponse.content[0].type === "text" ? xlResponse.content[0].text : "";
              const xlJsonMatch = xlText.match(/\{[\s\S]*\}/);
              if (xlJsonMatch) {
          ...
          // system = 全書類で共通（基本情報・案件整理・ルール・出力形式）→ cache_control で2書類目以降タダ
          // user = 書類固有（テンプレート本文）
          const markerSystem = `あなたは司法書士事務所の書類作成担当です。
［要入力_N《前案件:○○》］ と書かれた部分が「可変箇所」です。
...
${globalMemo ? `\n### 共通ルール\n${globalMemo}\n` : ""}
${memoText ? `\n### テンプレート注意事項\n${memoText}\n` : ""}
${renderQABlock("### ユーザー確定済みの質問と回答（質問文を含めた完全な文脈）")}
...
- **住所・氏名・法人名などが複数スロットに分割されている場合**: 基本情報には1つの連結した文字列しか無いので、**先頭のスロットに全体を入れ、後続の「建物名」「肩書続き」等のスロットは空文字 "" を返す**。要確認にしない。
  - 例: テンプレが《前案件:東京都渋谷区○○》《前案件:クレール西原１０２》で、今回の住所が「広島県大竹市西栄二丁目8番9号」（建物名なし）→ 先頭=住所全体、後続=""
  - 例: テンプレが《前案件:福田》《前案件:峻介》で今回が「三上春香」→ 先頭=全体、後続=""
- どこにも見つからない情報だけ "（要確認）"`;

          const markerUser = `## 書類テンプレート
${markedDocText}
${commentList ? `\n## コメント補足（［要入力_N］の意味のヒント）\n${commentList}\n` : ""}

この書類の ［要入力_N］ に対する値を JSON で返してください。`;

          // 抽出済みデータだけ送る（生ファイルは送らない＝コスト削減）
          console.log(`[produce/highlight] ${df.name}: dataContext length=${dataContext.length}, masterContent length=${(directMasterContent || "").length}, systemLen=${markerSystem.length}, userLen=${markerUser.length}`);
          ...
          const markerResponse = await client.messages.create({
            ...
            max_tokens: 4096,
            system: [
              { type: "text", text: markerSystem, cache_control: { type: "ephemeral" } },
            ],
            messages: [{ role: "user", content: markerUser }],
          });
          logTokenUsage("/api/document-templates/produce#docxMarker", MODEL, markerResponse.usage);
          ...
          for (let ci = 0; ci < replacementSets.length; ci++) {
            ...
            // filledSlots: slotId → 実際に埋めた値（+ ラベル）
            // replacements は { 元の値 → 新しい値 } の形式なので、docSlots（slotId → 元の値）
            // を使って slotId を正しく引き直す。Object.entries の key は元の値であって slotId ではない。
            const filledSlots: FilledSlotOut[] = [];
            for (const [slotId, origValue] of docSlots) {
              const newValue = replacements[origValue];
              if (newValue === undefined) continue;
              const meta = slotLabels.get(slotId);
              filledSlots.push({
                slotId,
                label: meta?.label || `slot_${slotId}`,
                value: String(newValue),
                format: meta?.format,
                sourceHint: meta?.sourceHint,
                copyIndex: replacementSets.length > 1 ? ci + 1 : undefined,
              });
            }
    ...
    const confirmedBlock = renderQABlock("## ユーザー確定済みの質問と回答（これらは絶対にこの値を使う。再解釈しない）");
...
データにない情報は "（要確認）" としてください。ただし上の「ユーザー確定済みの値」に該当するプレースホルダーは必ずその値を使うこと。`;
    ...
    try {
      ...
      logTokenUsage("/api/document-templates/produce#placeholder", MODEL, response.usage);
...
- ユーザー確定済みの値は必ずそのまま使うこと`;
              ...
              logTokenUsage("/api/document-templates/produce#markerReplace", MODEL, markerResponse.usage);
    ...
    async start(controller) {
      try {
        ...
        try {
          ...
          logTokenUsage("/api/document-templates/produce#fullgen", MODEL, final.usage);
        } catch { /* ignore */ }
