        ...
        try {
          ...
          if (ext === "xlsx" || ext === "xls") {
            ...
            if (xlPlaceholders.length === 0) {
              // プレースホルダーなし → 黄色セル方式
              try {
                const { extractXlsxMarkedCells, replaceXlsxMarkedCells, getXlsxMarkedText, expandYellowRowBlock } = await import("@/lib/xlsx-marker-parser");
                ...
                const xlMarkedText = getXlsxMarkedText(workingBuffer);
...
重要:
- **★が人名（代表取締役・取締役・株主氏名など）の場合、基本情報の役員リストや株主構成から該当する氏名を必ず使う。テンプレの人名（前案件）をそのまま返してはいけない**
- **★が会社名（署名欄・発行会社名など）の場合、基本情報の商号を使う**
- **\`__ROW_N_X__\` 形式のプレースホルダーは、その直前の表で追加された行（n番目の株主など）を示す。上の表の前行のパターンを参考に、株主構成などの配列データのn番目の値を返すこと**
- 数値セル（持株数・議決権数・割合）はそのまま数字で返す（全角変換はシステム側で行う）
- 基本情報・案件整理テキストに見つからない値だけ "（要確認）" とする`;
...
- **★が人名の場合の判断ルール（必ず守る）**:
  - 署名欄・記名押印欄の「代表取締役 ★氏名★」「取締役 ★氏名★」は、基本情報の「役員」リストから同じ役職の氏名を採用する
  - 例: テンプレが「代表取締役 ★福田峻介★」で今回の会社の代表取締役が「三上春香」なら、必ず {"福田峻介": "三上春香"} を返す
  - 同姓同名でない限り、テンプレの人名がそのまま今回の書類に残ることはない
  - 基本情報に対応する役員が見つからない場合のみ "（要確認）" とする
- **★が会社名（署名欄等）の場合**: 基本情報の「商号」で必ず差し替える
