...
以下はExcelの書類テンプレートです。［要入力_N］と書かれた部分が「可変箇所」です。
**［要入力_N］には前案件の値が入っていたのでAI側には見せていません。周辺の文脈と下記の案件データだけを頼りに値を決めてください。**
...
重要:
- **［要入力_N］ の全ての N について値を返すこと。省略しない**
- **周辺の文脈（直前の「代表取締役」「氏名又は名称」「住所」「持株」等の見出し）から何の値かを判断**
- **人名**: 「代表取締役 ［要入力_N］」は基本情報の役員リストから同役職の氏名を使う
- **会社名**: 署名欄・発行会社名は基本情報の商号を使う
- **株主表の行**: 基本情報の株主構成の n 番目を列ごとに展開（氏名・住所・持株数・比率・メール）
- **数値**（持株数・議決権数・割合）は数字のまま（全角変換はシステム側）
...
以下は書類テンプレートです。［要入力_N］ と書かれた部分が「可変箇所」で、あなたが値を埋めるべき場所です。
**［要入力_N］には前案件の値が入っていたのでAI側には見せていません。周辺の文脈と下記の案件データだけを頼りに値を決めてください。**
...
重要:
- **［要入力_N］ の全ての N について値を返すこと。省略しない**
- **周辺の文脈（直前の「代表取締役」「住所」「金」等の見出し）から何の値かを判断**
- **人名**: 「代表取締役 ［要入力_N］」の場合、基本情報の役員リストから役職=代表取締役の氏名を採用
- **会社名**: 署名欄の会社名は基本情報の商号を使用
- **株主の情報**: 基本情報の株主構成（氏名・住所・持株数・持株比率）から引く
- **日付**: 和暦全角（令和○年○月○日）で返す（数字も全角：１, ２, ３...）
- **金額・株数**: 全角カンマ区切り（１，２４９，９６３ 等）
