...
export async function POST(request: NextRequest) {
  ...
  // プロファイル（案件整理は directMasterContent または caseRoom 経由のみ。company.masterSheet は廃止）
  const caseRoom = caseRoomId ? company.caseRooms?.find(r => r.id === caseRoomId) : null;
  const masterSheet = caseRoom?.masterSheet; // caseRoom のみ（company.masterSheet は使わない）
  const profile = company.profile;

  const dataContext = JSON.stringify({
    基本情報: profile?.structured || {},
    ...(masterSheet?.structured && !directMasterContent ? { 案件情報: masterSheet.structured } : {}),
  }, null, 2);
  ...
  if (docxFiles.length > 0 && mode !== "generate") {
    ...
    if (allPlaceholders.size === 0) {
      ...
      // 原本ファイルを1回だけ読み込み（全テンプレで共有、コスト削減のため1回のみ送信）
      const { readCaseFiles, buildContentBlocks } = await import("@/lib/read-case-files");
      const caseFiles = await readCaseFiles(company, {
        folderPath: caseFolderPath,
        disabledFiles: caseDisabledFiles,
      });
      const hlSourceTexts = [...caseFiles.commonTexts, ...caseFiles.caseTexts];
      console.log(`[produce/highlight] source: ${hlSourceTexts.length} texts, ${caseFiles.pdfBlocks.length} PDFs, folder: ${caseFolderPath || "(sub.active)"}`);

      // 全テンプレの★マークテキストを収集して1回のAI呼び出しでまとめて処理
      docxFiles.sort((a, b) => a.name.localeCompare(b.name, "ja", { numeric: true }));

      // Word テンプレを収集
      type TemplateEntry = { name: string; baseName: string; markedText: string; markedFields: import("@/lib/docx-marker-parser").MarkedField[]; buffer: Buffer; isExcel: boolean; xlMarkedCells?: import("@/lib/xlsx-marker-parser").XlsxMarkedCell[] };
...
**★マーク★で囲まれた全ての値をキーとして返すこと。省略しない。テンプレの値をそのまま返さない。**
JSONのみ返してください。`;

                // content blocksでPDFも送る
                const xlBlocks = buildContentBlocks(caseFiles, xlMarkerPrompt);
                const xlResponse = await client.messages.create({
                  model: "claude-sonnet-4-6",
                  max_tokens: 4096,
                  messages: [{ role: "user", content: xlBlocks as Anthropic.ContentBlockParam[] }],
                });
...
- どこにも見つからない情報だけ "（要確認）" とすること`;
          ...
          // デバッグ: 最初のテンプレだけprompt内容を完全ダンプ
          if (df.name.includes("同意書") || df.name.includes("株主名簿") || df.name.includes("取締役決定")) {
            const fsDbg = await import("fs/promises");
            const dumpPath = `K:/recast/.claude/worktrees/reverent-shannon/tmp-prompt-${df.name.replace(/[\\/]/g, "_")}.txt`;
            try {
              await fsDbg.writeFile(dumpPath, markerPrompt, "utf-8");
              console.log(`[produce/highlight] ${df.name}: prompt dumped to ${dumpPath}, length=${markerPrompt.length}`);
            } catch (e) { console.log(`[produce/highlight] dump failed:`, e); }
          }
          const hlBlocks = buildContentBlocks(caseFiles, markerPrompt);
          console.log(`[produce/highlight] ${df.name}: sending ${hlBlocks.length} content blocks`);

          const markerResponse = await client.messages.create({
            model: "claude-sonnet-4-6",
            max_tokens: 4096,
            messages: [{ role: "user", content: hlBlocks as Anthropic.ContentBlockParam[] }],
          });
