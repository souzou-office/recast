...
## 重要: 同じ段落に複数 op を出さない (XML 破壊の元)

**1 つの paragraphIndex は deletes / rewrites のどちらか 1 つにしか入れない**:

NG 例:
\`\`\`json
"deletes": [{ "paragraphIndex": 18 }],
"rewrites": [{ "paragraphIndex": 18, "newText": "..." }]   // ← 段落 18 が重複してる
\`\`\`

判断基準:
- その段落を **削除したい** → deletes に追加 (rewrites には書かない)
- その段落の **本文を変えて残したい** → rewrites に追加 (deletes には書かない)

「議案2 を削除して 議案3 を 議案2 に繰り上げる」場合:
- 議案2 ブロック (段落 14〜19 等) は deletes に
- 議案3 ヘッダー (段落 21 等) は rewrites に "議案２　..." として
- → これが正しい使い分け
