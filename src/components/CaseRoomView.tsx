...
export default function CaseRoomView({ company, onUpdate }: Props) {
      ...
      <div className="h-full overflow-y-auto p-6">
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-[var(--color-fg)]">{company.name} の案件</h2>
            <button
          ...
          {caseRooms.length === 0 ? (
            <div className="text-center py-16 text-[var(--color-fg-subtle)]">
            ...
            <div className="space-y-2">
              {caseRooms.map(room => (
                <div key={room.id} className="rounded-xl border border-[var(--color-border)] hover:border-[var(--color-accent)]/30 transition-colors">
                    ...
                    >
                      {editingName === room.id ? (
                        <input
                          ...
                          className="text-base font-bold text-[var(--color-fg)] border-b border-[var(--color-accent)] outline-none w-full"
                        />
                      ) : (
                        <h3 className="text-base font-bold text-[var(--color-fg)]">{room.displayName}</h3>
                      )}
                      <div className="flex gap-2 mt-1">
                        {room.masterSheet && <span className="text-[9px] bg-green-100 text-[var(--color-ok-fg)] rounded px-1.5 py-0.5">案件整理済</span>}
                        {room.generatedDocuments && room.generatedDocuments.length > 0 && (
                          <span className="text-[9px] bg-[var(--color-accent-soft)] text-[var(--color-accent)] rounded px-1.5 py-0.5">書類{room.generatedDocuments.length}件</span>
                        )}
                        {room.checkResult && <span className="text-[9px] bg-purple-100 text-purple-600 rounded px-1.5 py-0.5">チェック済</span>}
                      </div>
                      <p className="text-[10px] text-[var(--color-fg-subtle)] mt-1">{new Date(room.updatedAt).toLocaleString("ja-JP")}</p>
                    </button>
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        onClick={() => { setEditingName(room.id); setEditNameValue(room.displayName); }}
                        className="text-[10px] text-[var(--color-fg-subtle)] hover:text-[var(--color-accent)] px-1"
                      >
                        名前変更
                      </button>
                      <button
                        onClick={() => handleDelete(room.id)}
                        className="text-[10px] text-[var(--color-fg-subtle)] hover:text-red-600 px-1"
                      >
                        削除
                      </button>
                    </div>
  ...
  return (
    <div className="flex flex-col h-full">
      {/* 案件部屋ヘッダー */}
      <div className="border-b border-[var(--color-border)] px-6 py-2 flex items-center gap-4 bg-[var(--color-panel)] shrink-0">
        <button
        ...
        <h3 className="text-sm font-bold text-[var(--color-fg)] flex-1 truncate">{selectedRoom.displayName}</h3>
        {/* サブタブ */}
