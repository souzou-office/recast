            ...
            {companyId && companyId !== "__search__" && (
              <button
                ...
                className={`text-[10px] transition-colors ${
                  showMasterJson ? "text-[var(--color-accent)] font-medium" : "text-[var(--color-fg-subtle)] hover:text-[var(--color-fg-muted)]"
                }`}
              >
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto p-4">
              <textarea
                ...
                className="w-full h-full min-h-[400px] rounded-lg bg-[var(--color-fg)] text-[var(--color-bg)] text-xs font-mono p-4 border-0 focus:outline-none resize-none leading-relaxed"
                spellCheck={false}
              />
            </div>
            {masterJsonDirty && (
              <div className="flex justify-end gap-2 px-4 py-2 border-t border-[var(--color-border)]">
                <button
                  onClick={() => { setMasterJsonDirty(false); setShowMasterJson(false); }}
                  className="rounded-lg px-3 py-1.5 text-xs text-[var(--color-fg-muted)] hover:bg-[var(--color-hover)]"
                >
                  キャンセル
                </button>
                <button
                  ...
                  className="rounded-lg bg-[var(--color-fg)] px-3 py-1.5 text-xs text-white hover:opacity-90"
                >
                  保存
                </button>
              </div>
          ...
          {isLoading &&
            messages[messages.length - 1]?.content === "" && (
              <div className="flex justify-start mb-4">
                <div className="bg-[var(--color-hover)] rounded-2xl px-4 py-3">
                  <div className="flex gap-1">
                    <span className="animate-bounce text-[var(--color-fg-subtle)]">●</span>
                    <span className="animate-bounce text-[var(--color-fg-subtle)] [animation-delay:0.15s]">●</span>
                    <span className="animate-bounce text-[var(--color-fg-subtle)] [animation-delay:0.3s]">●</span>
                  </div>
                </div>
              </div>
            )}
