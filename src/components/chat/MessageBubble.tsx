  ...
  if (isUser) {
    return (
      <div className="flex justify-end mb-6">
        <div className="max-w-[70%] rounded-2xl bg-[var(--color-fg)] px-4 py-3 text-sm text-white whitespace-pre-wrap">
          {message.content}
        </div>
      </div>
    );
      ...
      for (const part of parts) {
        ...
        newParts.push(
          <button
            ...
            className="text-[var(--color-accent)] hover:text-[var(--color-accent-fg)] hover:underline"
          >
            {matcher.name}
          </button>
        );
    ...
    <div className="mb-6">
      ...
      {streaming ? (
        ...
        <div className="prose prose-sm max-w-none text-[var(--color-fg)]
                        ...
                        prose-th:border prose-th:border-[var(--color-border)] prose-th:bg-[var(--color-hover)] prose-th:px-2 prose-th:py-1 prose-th:text-left prose-th:text-xs prose-th:font-medium prose-th:text-[var(--color-fg-muted)]
                        ...
                        prose-code:bg-[var(--color-hover)] prose-code:px-1 prose-code:rounded prose-code:text-sm
                        prose-pre:bg-[var(--color-fg)] prose-pre:text-gray-100 prose-pre:rounded-lg
                        prose-strong:text-[var(--color-fg)] prose-strong:font-semibold
                        prose-a:text-[var(--color-accent)]
                        prose-em:not-italic prose-em:text-[var(--color-warn-fg)] prose-em:text-sm">
